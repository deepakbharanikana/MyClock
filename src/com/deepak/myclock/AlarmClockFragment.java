...
import com.deepak.myclock.widget.ActionableToastBar;
import com.deepak.myclock.widget.TextTime;
...
public class AlarmClockFragment extends Fragment implements
	...
	public static String FRAGMENT_TAG = "AlarmClockFragment";
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mAddAlarmButton = (FloatingActionButton) v.findViewById(R.id.alarm_add_alarm);
		...
		mAddAlarmButton.setOnTouchListener(this);

		// Timeline layout only exists in tablet landscape mode for now.
		if (mTimelineLayout != null) {
			mTimelineView = (AlarmTimelineView) v
					.findViewById(R.id.alarm_timeline_view);
			mTimelineViewWidth = getActivity().getResources()
					.getDimensionPixelOffset(
							R.dimen.alarm_timeline_layout_width);
		}
		...
		mAdapter.registerDataSetObserver(new DataSetObserver() {
			...
			public void onChanged() {
				...
				if (count == 0) {
					/*mAddAlarmButton
							.setBackgroundResource(R.drawable.main_button_red);
*/
					// ...and if there exists a timeline view (currently only in
					// tablet landscape)
					if (mTimelineLayout != null && mAlarmsView != null) {

						// ...and if the previous adapter had alarms (indicating
						// a removal)...
						if (prevAdapterCount > 0) {
							...
							// and animate out the alarm timeline view,
							...
							ValueAnimator animator = ValueAnimator.ofFloat(1f,
									0f).setDuration(ANIMATION_DURATION);
							animator.setInterpolator(mCollapseInterpolator);
							animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
								@Override
								public void onAnimationUpdate(
										ValueAnimator animator) {
									Float value = (Float) animator
											.getAnimatedValue();
									int currentTimelineWidth = (int) (value * mTimelineViewWidth);
									float rightOffset = mTimelineViewWidth
											* (1 - value);
									mTimelineLayout
											.setTranslationX(rightOffset);
							...
							// If the previous adapter did not have alarms, no
							// animation needed,
							// just hide the timeline view and show the
							// "no alarms" icon.
					...
					/*mAddAlarmButton
							.setBackgroundResource(R.drawable.main_button_normal);*/
					...
					if (mTimelineLayout != null && mAlarmsView != null) {
						...
						mTimelineLayout.setVisibility(View.VISIBLE);
						// ...and if the previous adapter did not have alarms
						// (indicating an add)
						if (prevAdapterCount == 0) {
							...
							// and animate to show the timeline view, reducing
							// the width of the
							// alarms list / undo bar.
							ValueAnimator animator = ValueAnimator.ofFloat(0f,
									1f).setDuration(ANIMATION_DURATION);
							...
							animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
								...
								public void onAnimationUpdate(
									...
									mTimelineLayout.setAlpha(value);
									mTimelineLayout.requestLayout();
									((FrameLayout.LayoutParams) mAlarmsView
											.getLayoutParams()).setMargins(0,
											0, (int) -rightOffset, 0);
	...
	private void setUndoBarRightMargin(int margin) {
		FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) mUndoBar
				.getLayoutParams();
		((FrameLayout.LayoutParams) mUndoBar.getLayoutParams()).setMargins(
	...
	private void showUndoBar() {
		...
		}, 0, getResources().getString(R.string.alarm_deleted), true,
				R.string.alarm_undo, true);
	}
	...
	private void showLabelDialog(final Alarm alarm) {
		...
		final LabelDialogFragment newFragment = LabelDialogFragment
				.newInstance(alarm, alarm.label, FRAGMENT_TAG);
	...
	public class AlarmItemAdapter extends CursorAdapter {
		...
		public AlarmItemAdapter(Context context, long[] expandedIds,
			...
			mColorLit = res.getColor(R.color.blackish);
			mColorDim = res.getColor(R.color.clock_gray);
			mBackgroundColorExpanded = res.getColor(R.color.transparent);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				if (convertView.getTranslationX() != 0
						|| convertView.getTranslationY() != 0 || badConvertView) {
				...
				public boolean onPreDraw() {
					...
					for (int i = 0; i < list.getChildCount(); i++) {
						...
						if (startTop != top) {
							...
							child.animate().setDuration(ANIMATION_DURATION)
									.translationY(0);
		...
		public void bindView(final View view, Context context,
			...
			itemHolder.delete.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					try {
						view.animate().setDuration(ANIMATION_DURATION).alpha(0)
								.translationY(-1).withEndAction(new Runnable() {
									@Override
									public void run() {
				...
				public boolean onPreDraw() {
					...
					animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
						...
						public void onAnimationUpdate(ValueAnimator animator) {
							...
							itemHolder.hairLine
									.setTranslationY(-hairlineDistance * value);
					...
					animator.addListener(new AnimatorListener() {
						...
						public void onAnimationEnd(Animator animation) {
							...
							itemHolder.arrow.setRotation(180);
							itemHolder.hairLine
									.setTranslationY(-hairlineDistance);
						...
						@Override
						public void onAnimationStart(Animator animation) {
							itemHolder.hairLine.setBackgroundColor(getResources().getColor(R.color.accent));
						}
					});
					animator.start();
			...
			observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
				...
				public boolean onPreDraw() {
					...
					animator.addListener(new AnimatorListener() {
						...
						@Override
						public void onAnimationStart(Animator animation) {
							itemHolder.hairLine.setBackground(null);
						}
					});
					animator.start();
